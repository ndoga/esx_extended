...
AddEventHandler('chatMessage', function(playerId, author, message)
	...
	if message:sub(1, 1) == '/' and playerId > 0 then
		...
		TriggerClientEvent('nd_notify:Alert', source, "INFO", _U('commanderror_invalidcommand', commandName), 5000, 'error')
	end
end)
	...
	AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
		...
		if type == 'item_standard' then
			...
			if itemCount > 0 and sourceItem.count >= itemCount then
				if targetXPlayer.canCarryItem(itemName, itemCount) then
					...
					TriggerClientEvent('nd_notify:Alert', sourceXPlayer, "INFO", _U('ex_inv_lim', targetXPlayer.name), 5000, 'info')
					--targetXPlayer.showNotification(_U('received_item', itemCount, sourceItem.label, sourceXPlayer.name))
					TriggerClientEvent('nd_notify:Alert', targetXPlayer, "INFO",_U('received_item', itemCount, sourceItem.label, sourceXPlayer.name), 5000, 'info')
				else
				...
				TriggerClientEvent('nd_notify:Alert', sourceXPlayer, "INFO", _U('imp_invalid_quantity'), 5000, 'info')
			end
		elseif type == 'item_account' then
			if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
				...
				TriggerClientEvent('nd_notify:Alert', sourceXPlayer, "INFO", _U('gave_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName], targetXPlayer.name), 5000, 'info')
				...
				TriggerClientEvent('nd_notify:Alert', targetXPlayer, "INFO", _U('received_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName], sourceXPlayer.name), 5000, 'info')
			else
				--sourceXPlayer.showNotification(_U('imp_invalid_amount'))
				TriggerClientEvent('nd_notify:Alert', sourceXPlayer, "INFO", _U('imp_invalid_amount'), 5000, 'info')
			end
