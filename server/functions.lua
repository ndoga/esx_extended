...
function ESX.RegisterCommand(name, group, cb, allowConsole, suggestion)
	...
	if Core.RegisteredCommands[name] then
		print(('^7[^3WARNING^7] Command ^5"%s" ^7already registered, overriding command'):format(name))
		...
		if not command.allowConsole and playerId == 0 then
			...
			if error then
				if playerId == 0 then
					...
					TriggerClientEvent('nd_notify:Alert', xPlayer, "INFO", error, 5000, 'info')
				end
			else
				cb(xPlayer or false, args, function(msg)
					if playerId == 0 then
						...
						TriggerClientEvent('nd_notify:Alert', xPlayer, "INFO", msg, 5000, 'info')
					end
				end)
			end
		end
...
function ESX.UseItem(source, item, data)
	...
	if itemCallback then
	  itemCallback(source, item, data)
	else
	  print(('ESX.UseItem - item %s does not have a usable callback'):format(item))
	end
  end
